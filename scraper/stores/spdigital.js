 ...
 * SP Digital — WooCommerce Store API
 * Precio que muestra la API es precio tarjeta/Webpay.
 * Efectivo = tarjeta / 1.03 (descuento ~3% transferencia)
 */
...
class SPDigitalScraper extends BaseScraper {
  constructor() { super('spdigital', 'SP Digital'); }
  ...
  async scrapeCategory(slug, catId) {
    ...
    while (page <= 20) {
      ...
      this.log('info', `[spdigital] ${catId} pág ${page}`);
      try {
        ...
        this.stats.errors++;
        this.log('warn', `[spdigital] Error ${slug} pág ${page}: ${err.message}`);
        if (err.response?.status === 429) await new Promise(r => setTimeout(r, 15000));
        break;
      }
    }
    this.log('info', `✓ spdigital ${catId}: ${this.stats.found}`);
  }
}

if (require.main === module) {
  new SPDigitalScraper().run().then(r => { console.log('SPDigital:', r); process.exit(r.success ? 0 : 1); });
}
