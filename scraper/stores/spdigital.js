 ...
 * SPDigital — WooCommerce Store API con fallback a búsqueda HTML
 */
const BaseScraper = require('../base-scraper');
const cheerio = require('cheerio');

const BASE_URL    = 'https://www.spdigital.cl';
const BASE_API    = 'https://www.spdigital.cl/wp-json/wc/store/v1/products';
...
class SPDigitalScraper extends BaseScraper {
  ...
  async scrapeAll() {
    ...
    this.log('info', `[spdigital] Modo: ${this.useApi ? 'WooCommerce API' : 'HTML scraping'}`);
    ...
    for (const { slug, catId } of API_CATEGORIES) {
      ...
      while (page <= 20) {
        ...
        try {
          ...
          this.stats.errors++;
          this.log('warn', `[spdigital] API error ${slug} pág ${page}: ${err.message}`);
  ...
  async scrapeViaHtml() {
    ...
    for (const { query, catId, minPrice } of HTML_SEARCHES) {
      try {
        ...
        this.log('info', `[spdigital] HTML "${query}": ${newItems} productos`);
        await this.delay(2000, 3000);
      } catch(err) {
        this.log('warn', `[spdigital] HTML error "${query}": ${err.message}`);
      }
    }
  }
}
...
if (require.main === module) {
  new SPDigitalScraper().run().then(r => {
    console.log('SPDigital:', r); process.exit(r.success ? 0 : 1);
  });
}
