 ...
 * TruluStore — WooCommerce Store API
 */
...
class TruluStoreScraper extends BaseScraper {
  constructor() { super('trulustore', 'TruluStore'); }
  ...
  async scrapeCategory(slug, catId) {
    ...
    while (page <= 20) {
      ...
      this.log('info', `[trulustore] ${catId} pág ${page}`);
      try {
        ...
        this.stats.errors++;
        this.log('warn', `[trulustore] Error ${slug} pág ${page}: ${err.message}`);
        if (err.response?.status === 429) await new Promise(r => setTimeout(r, 15000));
        break;
      }
    }
    this.log('info', `✓ trulustore ${catId}: ${this.stats.found}`);
  }
}
