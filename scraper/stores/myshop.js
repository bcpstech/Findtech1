...
class MyShopScraper extends BaseScraper {
  constructor() { super('myshop', 'MyShop'); }
  ...
  async scrapeCategory(slug, catId) {
    ...
    while (page <= 20) {
      ...
      this.log('info', `[myshop] ${catId} pág ${page}`);
      try {
        ...
        this.stats.errors++;
        this.log('warn', `[myshop] Error ${slug} pág ${page}: ${err.message}`);
        if (err.response?.status === 429) await new Promise(r => setTimeout(r, 15000));
        break;
      }
    }
    this.log('info', `✓ myshop ${catId}: ${this.stats.found}`);
  }
}

if (require.main === module) {
  new MyShopScraper().run().then(r => { console.log('MyShop:', r); process.exit(r.success ? 0 : 1); });
}
