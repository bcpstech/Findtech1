...
class MegaBytesScraper extends BaseScraper {
  constructor() { super('megabytes', 'MegaBytes'); }
  ...
  async scrapeCategory(slug, catId) {
    ...
    while (page <= 20) {
      ...
      this.log('info', `[megabytes] ${catId} pág ${page}`);
      try {
        ...
        this.stats.errors++;
        this.log('warn', `[megabytes] Error ${slug} pág ${page}: ${err.message}`);
        if (err.response?.status === 429) await new Promise(r => setTimeout(r, 15000));
        break;
      }
    }
    this.log('info', `✓ megabytes ${catId}: ${this.stats.found}`);
  }
}
