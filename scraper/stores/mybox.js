...
class MyBoxScraper extends BaseScraper {
  ...
  async scrapeCategory(slug, catId) {
    ...
    while (page <= 20) {
      ...
      this.log('info', `[mybox] ${catId} pág ${page}`);
      try {
        ...
        this.stats.errors++;
        this.log('warn', `[mybox] Error ${slug} pág ${page}: ${err.message}`);
        if (err.response?.status === 429) await new Promise(r => setTimeout(r, 15000));
        break;
      }
    }
    this.log('info', `✓ mybox ${catId}: ${this.stats.found}`);
  }
}
