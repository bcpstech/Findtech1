 ...
 * MyBox — WooCommerce Store API con fallback a búsqueda HTML
 */
const BaseScraper = require('../base-scraper');
const cheerio = require('cheerio');

const BASE_URL    = 'https://www.mybox.cl';
...
class MyBoxScraper extends BaseScraper {
  ...
  async scrapeAll() {
    ...
    this.log('info', `[mybox] Modo: ${this.useApi ? 'WooCommerce API' : 'HTML scraping'}`);
    ...
    for (const { slug, catId } of API_CATEGORIES) {
      ...
      while (page <= 20) {
        ...
        try {
          ...
          this.stats.errors++;
          this.log('warn', `[mybox] API error ${slug} pág ${page}: ${err.message}`);
  ...
  async scrapeViaHtml() {
    ...
    for (const { query, catId, minPrice } of HTML_SEARCHES) {
      try {
        ...
        this.log('info', `[mybox] HTML "${query}": ${newItems} productos`);
        await this.delay(2000, 3000);
      } catch(err) {
        this.log('warn', `[mybox] HTML error "${query}": ${err.message}`);
      }
    }
  }
}
...
if (require.main === module) {
  new MyBoxScraper().run().then(r => {
    console.log('MyBox:', r); process.exit(r.success ? 0 : 1);
  });
}
