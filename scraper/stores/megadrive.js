 ...
 * MegaDrive Store — PrestaShop, URL: megadrivestore.cl/138-procesadores
 */
require('dotenv').config();
const BaseScraper = require('../base-scraper');
const cheerio = require('cheerio');

const BASE      = 'https://megadrivestore.cl';
...
const CATEGORIES = [
  { url: '/138-procesadores',       catId: 'cpu'     },
...
class MegaDriveScraper extends BaseScraper {
  ...
  async scrapeCategory(cat) {
    ...
    while (page <= 20) {
      ...
      } catch (err) { this.log('warn', `[megadrive] HTTP: ${err.message}`); break; }

      const items = $('article.product-miniature, li.ajax_block_product, .products .product-miniature');
      if (!items.length) { this.log('info', `[megadrive] Sin productos pág ${page}`); break; }
      ...
      for (const el of items.toArray()) {
        ...
        } catch (err) { this.log('warn', `[megadrive] item: ${err.message}`); }
      }
      total += newInPage;
      this.log('info', `[megadrive] ✓ pág ${page}: ${newInPage}`);
      if (items.length < 6 || newInPage === 0) break;
      page++; await this.delay(1500, 2500);
    }
    this.log('info', `[megadrive] ✓ ${cat.url}: ${total} total`);
  }
}
