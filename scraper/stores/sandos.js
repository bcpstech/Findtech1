...
class SandosScraper extends BaseScraper {
  constructor() { super('sandos', 'Sandos'); }
  ...
  async scrapeCategory(slug, catId) {
    ...
    while (page <= 20) {
      ...
      this.log('info', `[sandos] ${catId} pág ${page}`);
      try {
        ...
        this.stats.errors++;
        this.log('warn', `[sandos] Error ${slug} pág ${page}: ${err.message}`);
        if (err.response?.status === 429) await new Promise(r => setTimeout(r, 15000));
        break;
      }
    }
    this.log('info', `✓ sandos ${catId}: ${this.stats.found}`);
  }
}

if (require.main === module) {
  new SandosScraper().run().then(r => {
    console.log('Sandos:', r);
