 ...
 * TYTGamer — WooCommerce Store API con fallback a búsqueda HTML
 */
const BaseScraper = require('../base-scraper');
const cheerio = require('cheerio');

const BASE_URL    = 'https://www.tytgamer.cl';
...
class TYTGamerScraper extends BaseScraper {
  ...
  async scrapeAll() {
    ...
    this.log('info', `[tytgamer] Modo: ${this.useApi ? 'WooCommerce API' : 'HTML scraping'}`);
    ...
    for (const { slug, catId } of API_CATEGORIES) {
      ...
      while (page <= 20) {
        ...
        try {
          ...
          this.stats.errors++;
          this.log('warn', `[tytgamer] API error ${slug} pág ${page}: ${err.message}`);
  ...
  async scrapeViaHtml() {
    ...
    for (const { query, catId, minPrice } of HTML_SEARCHES) {
      try {
        ...
        this.log('info', `[tytgamer] HTML "${query}": ${newItems} productos`);
        await this.delay(2000, 3000);
      } catch(err) {
        this.log('warn', `[tytgamer] HTML error "${query}": ${err.message}`);
      }
    }
  }
}
...
if (require.main === module) {
  new TYTGamerScraper().run().then(r => {
    console.log('TYTGamer:', r); process.exit(r.success ? 0 : 1);
  });
}
