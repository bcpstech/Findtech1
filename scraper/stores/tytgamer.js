 ...
 * TYTGamer — WooCommerce Store API
 */
...
class TYTGamerScraper extends BaseScraper {
  constructor() { super('tytgamer', 'TYTGamer'); }
  ...
  async scrapeCategory(slug, catId) {
    ...
    while (page <= 20) {
      ...
      this.log('info', `[tytgamer] ${catId} pág ${page}`);
      try {
        ...
        this.stats.errors++;
        this.log('warn', `[tytgamer] Error ${slug} pág ${page}: ${err.message}`);
        if (err.response?.status === 429) await new Promise(r => setTimeout(r, 15000));
        break;
      }
    }
    this.log('info', `✓ tytgamer ${catId}: ${this.stats.found}`);
  }
}

if (require.main === module) {
  new TYTGamerScraper().run().then(r => {
    console.log('TYTGamer:', r);
