...
class Dust2Scraper extends BaseScraper {
  constructor() { super('dust2', 'Dust2'); }
  ...
  async scrapeCategory(slug, catId) {
    ...
    while (page <= 20) {
      ...
      this.log('info', `[dust2] ${catId} pág ${page}`);
      try {
        ...
        this.stats.errors++;
        this.log('warn', `[dust2] Error ${slug} pág ${page}: ${err.message}`);
        if (err.response?.status === 429) await new Promise(r => setTimeout(r, 15000));
        break;
      }
    }
    this.log('info', `✓ dust2 ${catId}: ${this.stats.found}`);
  }
}
