 ...
 *
 * Para dominios bloqueados, enruta la descarga por el proxy CF Worker.
 */
...
// FIX: agregar centrale.cl — bloquea hotlinking con 403
const PROXY_DOMAINS = ['n1g.cl', 'www.n1g.cl', 'centrale.cl', 'www.centrale.cl'];

const _uploaded = new Set();

// ── Firma AWS-S3 para R2 ──────────────────────────────────────────────────
...
async function uploadToR2(buffer, key, contentType) {
  ...
  await axios.put(url, buffer, { headers, maxBodyLength: 10 * 1024 * 1024, timeout: 30000 });
...
async function downloadImage(sourceUrl, retries = 2) {
  ...
  const headers = viaProxy
    ? { 'User-Agent': 'Mozilla/5.0 FindTech/1.0' }
    : {
...
async function mirrorImage(sourceUrl, slug) {
  ...
  try {
    ...
    if (buffer.length < 500) {
      console.warn(`[R2] Imagen vacía (${buffer.length}B) — saltando`);
      return null;
    }
